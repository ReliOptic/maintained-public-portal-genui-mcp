...
4. 컴포저가 **Action 카드**(정부24·홈택스 핸드오프) + **Insight Rail** + **Evidence Rail** + **Data Sections**(CI가 미리 가져온 공공 API 스냅샷)를 하나의 artifact로 조립합니다.
...
- MVP scheduled adapter `welfare-facility-kr`: CI/maintainer 환경에서 복지시설 현황을 pre-fetch하고, 일반 사용자 런타임은 번들된 snapshot만 읽습니다.
...
## Maintainer 전용: scheduled adapter pre-fetch

v0.2의 복지시설 Data Sections는 런타임 실시간 호출이 아니라 `npm run compile` 시점에 생성된 `compiled.sqlite` snapshot에서 읽습니다. 일반 사용자는 API 키를 설정할 필요가 없고, 사용자 기기에 공공데이터포털 인증키를 저장하지 않습니다.

실데이터 snapshot을 갱신하는 maintainer/CI 환경에서만 `WELFARE_API_KEY`를 설정합니다. 키가 없으면 compile 단계는 `call_status: "mock"` 샘플 record를 생성해 로컬 개발과 패키징 검증을 가능하게 합니다.
...
- 일반 사용자 런타임은 번들된 snapshot을 읽습니다. maintainer/CI compile에서 `WELFARE_API_KEY`가 없으면 복지시설 Data Sections는 `call_status: "mock"` 샘플 데이터입니다.
